...
win.on("scroll", function (event) {
    ...
    allMods.each(function (i, el) {
        ...
        if (el.visible(true)) {
            ...
            } else if (el[0].classList.contains("to-slide-top")) {
                el.addClass("slide-in-from-top");
            } else if (el[0].classList.contains("to-spin") && !(el[0].classList.contains("spin") || el[0].classList.contains("already-visible"))) {
                var id = el.attr('id')
                var idNum = id[id.length -1];
                $('#navbar-icon-' + idNum).addClass("spin");
                el.addClass("spin");
            }
        }
    });
